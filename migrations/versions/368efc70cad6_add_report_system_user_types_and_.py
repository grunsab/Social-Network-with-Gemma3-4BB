...
def upgrade():
    ...
    op.execute("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'commentvisibility') THEN CREATE TYPE commentvisibility AS ENUM ('PUBLIC', 'FRIENDS_ONLY', 'HIDDEN'); END IF; END $$;")
    op.execute("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'usertype') THEN CREATE TYPE usertype AS ENUM ('USER', 'ADMIN'); END IF; END $$;")
    op.execute("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'reportcontenttype') THEN CREATE TYPE reportcontenttype AS ENUM ('POST', 'COMMENT', 'AMPERSOUND'); END IF; END $$;")
    op.execute("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'reportstatus') THEN CREATE TYPE reportstatus AS ENUM ('PENDING', 'RESOLVED_AUTO', 'RESOLVED_MANUAL', 'DISMISSED'); END IF; END $$;")
